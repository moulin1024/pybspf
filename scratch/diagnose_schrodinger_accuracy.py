...
def run(n, degree, endpoint, strong=False, n_basis=24, points=None):
 ...
 p=bspf_plans.plan_1d(x,**opts)
 weak=bspf_galerkin.galerkin_1d(p,constraints=((0,1),(1,1)) if strong else ())
 ...
 interp=float(j.max(j.abs(bspf_calculus.interpolate(p,mode,z)-j.cos(32*j.pi*z/20))))
