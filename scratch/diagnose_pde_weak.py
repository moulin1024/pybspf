...
def resolved(plan, weak, order, gauss_order=8):
    ...
    split=b.decompose(plan,weak.extension)
    spectrum=j.fft.fft(split.residual,axis=0)/plan.x.size
    phase=j.exp(1j*(z[:,None]-plan.x[0])*plan.omega)
    Q=b.basis_matrix(plan.knots,z,degree=plan.degree)@split.coefficients+(phase@spectrum).real
    G=b.basis_matrix(plan.knots,z,degree=plan.degree,derivative=order)@split.coefficients+(phase@((1j*plan.omega[:,None])**order*spectrum)).real
...
if __name__=='__main__':
 for model in ('beam','schrodinger'):
  for n in (33,65,129):
   ...
   p=b.plan_1d(j.linspace(0,length,n),degree=5,n_basis=16,boundary_points=7)
   k=2 if model=='beam' else 1
   weak=b.galerkin_1d(p,derivative_order=k,constraints=((0,0),(0,1)) if k==2 else ())
