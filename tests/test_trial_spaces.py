...
def test_matches_frozen_pre_migration_space():
    from pathlib import Path
    with np.load(Path(__file__).parent / 'reference/closed_line.npz', allow_pickle=False) as ref:
        line = ClosedBSPFLine(33)
        for actual, key in [(line.scalar_values(ref['points'], 2), 'scalar'),
                            (line.tangent_values(ref['points'], 2), 'tangent'),
                            (line.values(ref['points'], 2), 'normal')]:
            np.testing.assert_allclose(actual, ref[key], rtol=2e-10, atol=2e-9)
        np.testing.assert_allclose(line.derivative_map, ref['derivative_map'], rtol=2e-10, atol=2e-9)
