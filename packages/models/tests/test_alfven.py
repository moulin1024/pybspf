...
def grid(n=33):
    ...
    return x, bspf_plans.plan_1d(x, degree=7, n_basis=16, boundary_points=9)


def test_variable_density_mass_and_energy():
    x, spatial = grid()
    p = bspf_alfven.plan_alfven(spatial, density=lambda z: 1+z, magnetic_field=2., permeability=2.)
    ones = jnp.ones_like(x)
    np.testing.assert_allclose(ones@p.mass@ones, 1.5, atol=1e-11)
    # xi=z, velocity=1: E=1/2 int (1+z+2) dz=1.75.
    np.testing.assert_allclose(bspf_alfven.alfven_energy(p, x, ones), 1.75, atol=1e-10)
    # xi=z², velocity=1: boundary power=2*(2-0)=4.
    np.testing.assert_allclose(bspf_alfven.alfven_boundary_power(p, x*x, ones), 4., atol=1e-9)
...
def test_uniform_normal_mode_with_physical_scaling_and_jit():
    ...
    p = bspf_alfven.plan_alfven(spatial, density=2., magnetic_field=2., permeability=2.)
    times = jnp.array([0., .4, 1., 2.])
    initial = jnp.sin(jnp.pi*x).at[jnp.array([0, -1])].set(0.)
    run = jax.jit(lambda p, q: bspf_alfven.integrate_alfven(p, q, 0*q, times,
...
def test_validation_and_single_time():
    ...
    for density in (0., -1., jnp.nan, lambda z: -jnp.ones_like(z)):
        with pytest.raises(ValueError, match='density'):
            bspf_alfven.plan_alfven(spatial, density=density)
    for kwargs in ({'magnetic_field':0.}, {'permeability':-1.}):
        with pytest.raises(ValueError):
            bspf_alfven.plan_alfven(spatial, **kwargs)
    p = bspf_alfven.plan_alfven(spatial)
    bc = lambda t: jnp.zeros(2)
    q, v = bspf_alfven.integrate_alfven(p, 0*x, 0*x, jnp.array([0.]), boundary=bc)
    np.testing.assert_array_equal(q, jnp.zeros((1, x.size)))
    np.testing.assert_array_equal(v, q)
    with pytest.raises(ValueError, match='substeps'):
        bspf_alfven.integrate_alfven(p, 0*x, 0*x, jnp.array([0.]), boundary=bc, substeps=0)
    with pytest.raises(ValueError, match='two real'):
        bspf_alfven.integrate_alfven(p, 0*x, 0*x, jnp.array([0.]), boundary=lambda t: jnp.zeros(3))
...
def test_off_grid_derivative_preserves_original_interpolant():
    ...
    result = jax.jit(lambda f: bspf_calculus.interpolate(spatial, f, points, derivative=1))(values)
    np.testing.assert_allclose(result, .3*jnp.exp(.3*points)+2*jnp.cos(2*points), atol=2e-8)
    np.testing.assert_allclose(bspf_calculus.interpolate(spatial, values, x, derivative=1),
                               bspf_operators.differentiate(spatial, values), atol=2e-11)
    for order in (-1, True, 1.5, 20):
        with pytest.raises(ValueError, match='derivative'):
            bspf_calculus.interpolate(spatial, values, points, derivative=order)
