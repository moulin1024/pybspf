...
def plan():
    pressure = bspf_pressure.plan_pressure_poisson2d(
        np.linspace(-1, 1, 24),
        np.linspace(-1, 1, 25),
        q=3,
        degree=5,
    ...
    return bspf_navier_stokes.plan_navier_stokes2d(pressure, closure="sbp84", viscosity=0.02)
...
def test_direct_projection_against_dense_constraint_svd(plan):
    ...
    projected, diagnostics = jax.jit(bspf_navier_stokes.ns_project_velocity)(plan, jnp.asarray(raw))
    ...
    assert np.max(abs(np.asarray(bspf_navier_stokes.ns_divergence(plan, projected)))) < 1e-10
    again, _ = bspf_navier_stokes.ns_project_velocity(plan, projected)
...
def test_nonlinear_energy_law_and_uniform_flow(plan):
    ...
    u, _ = bspf_navier_stokes.ns_project_velocity(plan, raw)
    acceleration, diag = jax.jit(bspf_navier_stokes.ns_rhs)(plan, u)
    ...
    linear = jax.jvp(lambda v: bspf_navier_stokes.ns_rhs(plan, v)[0], (base,), (u,))[1]
    np.testing.assert_allclose(
        jnp.sum(weights[..., None] * u * linear), dissipation, rtol=2e-12, atol=1e-10
    )
    force = -bspf_navier_stokes.ns_raw_rhs(plan, base)
    updated, _ = jax.jit(bspf_navier_stokes.ns_rk4_step)(plan, base, 1e-3, force)
...
def test_invalid_grid_and_closure(plan):
    ...
    with pytest.raises(ValueError, match="closure"):
        bspf_navier_stokes.plan_navier_stokes2d(plan.pressure, closure="invalid")
