...
import bspf_models.kinetic.vlasov_poisson as bspf_vlasov_poisson
...
def grids():
    ...
    zp = bspf_plans.plan_1d(z, degree=5, n_basis=12, boundary_points=7)
    vp = bspf_plans.plan_1d(v, degree=5, n_basis=16, boundary_points=7)
    return z, v, zp, vp
...
def test_poisson_primitives_nonzero_dirichlet_and_batch():
    ...
    phi, field = jax.jit(lambda s: bspf_vlasov_poisson.poisson_dirichlet(
...
def test_maxwellian_reservoir_is_stationary_under_jit():
    z, v, zp, vp = grids()
    p = bspf_vlasov_poisson.plan_vlasov_poisson(zp, vp)
    f0 = jnp.broadcast_to(p.background, (z.size, v.size))
    times = jnp.array([0., .1, .3])
    run = jax.jit(lambda p, f: bspf_vlasov_poisson.integrate_vlasov_poisson(p, f, times, substeps=50))
    f, phi, e = run(p, f0)
...
def test_density_to_field_sign_and_boundaries():
    ...
    phi, e = bspf_vlasov_poisson.vlasov_poisson_fields(p, f)
    expected, expected_e = bspf_vlasov_poisson.poisson_dirichlet(zp, .01*z)
    ...
    with pytest.raises(ValueError, match='temperature'):
        bspf_vlasov_poisson.plan_vlasov_poisson(zp, vp, temperature=0.)
...
def test_perturbation_evolves_with_self_consistent_electron_force():
    ...
    f, phi, e = bspf_vlasov_poisson.integrate_vlasov_poisson(p, f0, times, substeps=50)
    refined, _, _ = bspf_vlasov_poisson.integrate_vlasov_poisson(p, f0, times, substeps=100)
    np.testing.assert_allclose(f, refined, atol=2e-11)
    expected_phi, expected_e = bspf_vlasov_poisson.vlasov_poisson_fields(p, f)
    np.testing.assert_allclose(e, expected_e, atol=2e-14)
    np.testing.assert_allclose(phi, expected_phi, atol=2e-14)
    assert float(jnp.max(jnp.abs(e[-1]-e[0]))) > 1e-8
    energy = bspf_calculus.integrate(zp, (e*e).T)
