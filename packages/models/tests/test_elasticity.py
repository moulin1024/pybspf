...
import bspf_models.waves.elasticity as bspf_elasticity
...
import numpy as np
import pytest
from bspf_models.waves.references import cantilever_modes
from bspf_models.waves.references import cantilever_spectrum
from bspf_models.waves.references import cantilever_step_response


def test_forced_oscillators_nonuniform_times_and_jit():
    weights = jnp.array([2., 3.])
    gradient = jnp.diag(jnp.array([2., 3.]))
    weak = bspf_galerkin.Galerkin1D(mass=jnp.diag(weights), stiffness=gradient.T@jnp.diag(weights)@gradient,
    ...
    q, v = jax.jit(lambda w: bspf_elasticity.integrate_elastic(w, initial, velocity, times,
...
def test_zero_frequency_constant_acceleration():
    ...
    weak = bspf_galerkin.galerkin_1d(bspf_plans.plan_1d(x, degree=3, n_basis=8, boundary_points=5), quadrature_order=6)
    ...
    q, v = bspf_elasticity.integrate_elastic(weak, initial, velocity, times, force=force)
...
def test_beam_static_fundamental_frequency_and_transient(strong_free):
    ...
    weak = bspf_galerkin.galerkin_1d(plan, derivative_order=2, constraints=constraints, quadrature_order=8)
    force = weak.values.T@weak.quadrature_weights
    frequency, modes = bspf_elasticity.elastic_modes(weak)
    ...
    q, _ = bspf_elasticity.integrate_elastic(weak, jnp.zeros(weak.mass.shape[0]), jnp.zeros(weak.mass.shape[0]), times, force=force)
