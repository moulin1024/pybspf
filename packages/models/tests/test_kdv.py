...
def setup():
    ...
    spatial = bspf_plans.plan_1d(x, degree=3, n_basis=8, boundary_points=5)
    return x, spatial, bspf_kdv.plan_kdv(spatial, quadrature_order=6)


def test_homogeneous_linear_operator_is_dissipative():
    x, spatial, kdv = setup()
    derivative = bspf_operators.differentiate(spatial, jnp.eye(x.size))
...
def test_time_dependent_nonperiodic_cubic_linear_solution():
    ...
    out = jax.jit(lambda p: bspf_kdv.integrate_kdv(
    ...
    np.testing.assert_allclose(bspf_operators.differentiate(spatial, out.T)[-1], 3., atol=1e-9)
...
def test_initial_output_and_arguments():
    ...
    out = bspf_kdv.integrate_kdv(kdv, x, jnp.array([0.]), boundary=boundary)
    np.testing.assert_array_equal(out[0], x)
    for steps in (0, True, 1.5):
        with pytest.raises(ValueError, match='substeps'):
            bspf_kdv.integrate_kdv(kdv, x, jnp.array([0., 1.]), boundary=boundary, substeps=steps)
    with pytest.raises(ValueError, match='real vector'):
        bspf_kdv.integrate_kdv(kdv, x+0j, jnp.array([0.]), boundary=boundary)
    with pytest.raises(ValueError, match='three real'):
        bspf_kdv.integrate_kdv(kdv, x, jnp.array([0.]), boundary=lambda t: jnp.zeros(2))
