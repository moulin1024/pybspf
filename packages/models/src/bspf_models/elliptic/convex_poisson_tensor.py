...
from bspf_models.elliptic.convex_poisson import ConvexPoissonSolution
...
def _implementation_key(kind):
    ...
    sources = [
        "bspf_models._numerics.trial_spaces",
        "bspf_models._numerics._weak_basis",
        "bspf_models._numerics._gpu_basis",
        "pybspf.basis", "pybspf.plans", "pybspf.operators", "pybspf.endpoints",
        "bspf_models._numerics._compressed_transform",
    ]
    if kind != "line":
        sources += ["bspf_models.elliptic.embedded_poisson", "bspf_models.elliptic.convex_poisson"]
        h.update(b"tensor-geometry-v1")
    if kind == "coarse":
        sources += ["bspf_models.elliptic.convex_poisson_tensor"]
    for name in sources:
        path = Path(import_module(name).__file__)
        h.update(name.encode())
        h.update(path.read_bytes())
    try:
        import gmpy2
    except ModuleNotFoundError as exc:
        raise ImportError("MPFR setup requires bspf-models[precision]") from exc
