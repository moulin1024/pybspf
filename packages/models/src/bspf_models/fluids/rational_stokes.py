...
class RationalBasis:
    ...
    def __init__(self, z, degree, poles, laurent, *, device=None):
        ...
        if device is not None:
            import jax
            from bspf_models.fluids._gpu_rational import construct_block
    ...
    def evaluate_gpu(self, z, device):
        ...
        import jax
        from bspf_models.fluids._gpu_rational import evaluate_blocks
        from bspf_models.fluids._gpu_rational import unpad_blocks
        if device.platform != "gpu" or not jax.config.x64_enabled:
...
class RationalStokesExtension:
    ...
    def _evaluate_gpu(self, points, coefficients, batch_size, device, return_device):
        ...
        from bspf_models.fluids._immersed_assembly import _join_rational_chunks
        from bspf_models.fluids._gpu_rational import stream_rows
