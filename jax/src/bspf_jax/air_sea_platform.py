...
CLI: python -m bspf_jax.air_sea_platform {run,resume,report,validate} ...
...
from .surface_exchange import SurfaceExchangeConfig, REFERENCE_COMMIT, coare35
...
def provenance():
    ...
    return dict(
        ...
        dependencies={
            ...
            for name in ("jax", "jaxlib", "numpy", "scipy", "gmpy2", "netCDF4")
        },
...
def resume(path, until=None):
    ...
    for key in ("source_files", "python", "dependencies", "backend", "float64"):
        if checkpoint.provenance[key] != current[key]:
...
def main():
    ...
    if args.command == "run":
        ...
        from .air_sea_validation import validate
