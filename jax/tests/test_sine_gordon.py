...
def system(n=33):
    ...
    return x, b.galerkin_1d(p, quadrature_order=8)
...
def test_boundary_acceleration_lift_under_jit():
    ...
    run = jax.jit(lambda w, u, v: b.integrate_sine_gordon(
...
def test_single_time_and_invalid_arguments():
    ...
    u, v = b.integrate_sine_gordon(weak, initial, 0*initial, jnp.array([0.]), boundary=boundary)
    np.testing.assert_array_equal(u[0], initial)
    np.testing.assert_array_equal(v[0], 0*initial)
    for steps in (0, True, 1.5):
        with pytest.raises(ValueError, match='substeps'):
            b.integrate_sine_gordon(weak, initial, 0*initial, jnp.array([0.]),
                                    boundary=boundary, substeps=steps)
    with pytest.raises(ValueError, match='two real'):
        b.integrate_sine_gordon(weak, initial, 0*initial, jnp.array([0.]), boundary=lambda t: jnp.ones(3))
    with pytest.raises(ValueError, match='velocity'):
        b.integrate_sine_gordon(weak, initial, jnp.ones(2), jnp.array([0.]), boundary=boundary)
