...
def test_basis_matches_scipy(degree):
    ...
    np.testing.assert_allclose(b.basis_matrix(t, x, degree=degree).sum(axis=1), 1, atol=1e-14)


def test_fit_kkt_and_reconstruction(plan):
    f = jnp.sin(3*plan.x) + .1*plan.x
    result = b.decompose(plan, f)
    np.testing.assert_allclose(result.spline+result.residual, f, atol=1e-14)
    np.testing.assert_allclose(plan.constraint@result.coefficients, b.endpoint_jets(plan, f).reshape(-1), atol=1e-9)


def test_polynomial_derivatives_and_complex_batch(plan):
    x = plan.x
    f = jnp.stack([x**3, 1j*x**3], axis=-1)
    result = jax.jit(partial(b.derivatives, orders=(0,1,2,3,4)))(plan, f)
...
def test_vmap_and_grad(plan):
    ...
    fn = jax.jit(jax.vmap(lambda v: b.differentiate(plan,v)))
    np.testing.assert_allclose(fn(batch), jnp.stack([b.differentiate(plan,v) for v in batch]),atol=1e-10)
    energy = lambda v: jnp.sum(b.differentiate(plan,v)**2)
...
def test_regularization_is_differentiable(plan):
    f=jnp.sin(7*plan.x)
    fun=lambda lam:jnp.sum(b.decompose(b.with_regularization(plan,lam),f).spline**2)
...
def test_interpolation_and_primitives(plan):
    ...
    np.testing.assert_allclose(jax.jit(b.interpolate)(plan,f,q),q**3+2*q+1j*q**2,atol=1e-11)
    primitive=x**4/4+x**2+1j*x**3/3
    expected=primitive-primitive[0]+2
    np.testing.assert_allclose(jax.jit(b.antiderivative)(plan,f,left_value=2),expected,atol=1e-11)
    integral=jax.jit(b.integrate)(plan,f,a=-.4,b=.7)
...
def test_integral_bounds_ad(plan):
    f=jnp.cos(plan.x)
    F=lambda t:b.integrate(plan,f,a=-.4,b=t)
    np.testing.assert_allclose(jax.grad(F)(.3),b.interpolate(plan,f,jnp.array([.3]))[0],atol=1e-9)
    np.testing.assert_allclose(b.integrate(plan,f,a=.4,b=-.2),-b.integrate(plan,f,a=-.2,b=.4),atol=1e-12)
    assert jnp.isnan(b.integrate(plan,f,a=-2,b=0))


@pytest.mark.parametrize("dimension", [2,3])
def test_tensor_calculus(dimension):
    coords=tuple(jnp.linspace(-1,1,n) for n in (17,19,21)[:dimension])
    p=b.tensor_plan(*(b.plan_1d(x,degree=3,n_basis=8,boundary_points=5) for x in coords))
    mesh=jnp.meshgrid(*coords,indexing='ij')
    f=sum(x**2 for x in mesh)
    np.testing.assert_allclose(jax.jit(b.laplacian)(p,f),2*dimension,atol=1e-8)
    g=jax.jit(b.gradient)(p,f)
    np.testing.assert_allclose(g,jnp.stack([2*x for x in mesh]),atol=1e-9)
    np.testing.assert_allclose(b.divergence(p,g),2*dimension,atol=1e-8)
    np.testing.assert_allclose(b.curl(p,g),0,atol=1e-8)
    ...
    components=jax.jit(b.tensor_decompose)(p,f)
    assert len(components)==2**dimension
    np.testing.assert_allclose(sum(components.values()),f,atol=1e-12)
    np.testing.assert_allclose(jax.jit(b.integrate_box)(p,f),dimension*2**dimension/3,atol=1e-10)
    new=tuple(jnp.linspace(-1,1,9+i) for i in range(dimension))
    exact=sum(x**2 for x in jnp.meshgrid(*new,indexing='ij'))
    np.testing.assert_allclose(jax.jit(b.interpolate_grid)(p,f,new),exact,atol=1e-10)


def test_boundary_jets(plan):
    f=plan.x**2
    bc=jnp.array([[1.,-2.,2.,0.],[1.,2.,2.,0.]])
    s=jax.jit(b.decompose)(plan,f,boundary=bc)
    np.testing.assert_allclose(plan.constraint@s.coefficients,bc.reshape(-1),atol=1e-10)


def test_no_correction(plan):
    f=jnp.sin(6*plan.x)
    s=b.decompose(plan,f)
    np.testing.assert_allclose(b.differentiate(plan,f,correction=False),plan.basis[1]@s.coefficients)


@pytest.mark.parametrize("x", [[0,0,1],[0,2,1],[0,1,3],[0,np.nan,2],[[0,1],[2,3]]])
def test_bad_grid(x):
    with pytest.raises(ValueError):b.plan_1d(x)


def test_shape_and_order_errors(plan):
    with pytest.raises(ValueError):b.differentiate(plan,jnp.ones(64))
    with pytest.raises(ValueError):b.differentiate(plan,jnp.ones(65),order=1.5)
    with pytest.raises(ValueError):b.differentiate(plan,jnp.ones(65),axis=1)
    with pytest.raises(ValueError):b.plan_1d(plan.x,lam=-1)
...
def test_nonpolynomial_matches_independent_scipy_kkt(n,complex_input):
    ...
    p=b.plan_1d(x,degree=3,n_basis=10,constraint_order=2,boundary_points=5,lam=.002)
    ...
    for k in (1,2,3,4):
        ...
        np.testing.assert_allclose(b.differentiate(p,f,order=k),expected,atol=2e-7,rtol=2e-8)


def test_zero_constraints_and_empty_batches():
    p=b.plan_1d(jnp.linspace(0,1,17),degree=3,n_basis=6,constraint_order=0)
    f=jnp.ones((17,0))
    assert b.differentiate(p,f).shape == f.shape
    np.testing.assert_allclose(b.differentiate(p,jnp.ones(17)),0,atol=1e-11)
...
def test_second_primitive_and_nonzero_constants(plan):
    ...
    np.testing.assert_allclose(jax.jit(partial(b.antiderivative,order=2))(plan,f,left_value=2,left_slope=4),expected,atol=1e-10)


def test_nonpolynomial_interpolant_and_integral_consistency(plan):
    f=jnp.sin(13*plan.x)+.2*plan.x
    np.testing.assert_allclose(b.interpolate(plan,f,plan.x),f,atol=1e-12)
    # Fundamental theorem for the SAME interpolant, including a nontrivial FFT residual.
    q=.271
    derivative=jax.jacfwd(lambda t:b.interpolate(plan,f,jnp.array([t]))[0])(q)
    eps=1e-5
    fd=(b.interpolate(plan,f,jnp.array([q+eps]))[0]-b.interpolate(plan,f,jnp.array([q-eps]))[0])/(2*eps)
    np.testing.assert_allclose(derivative,fd,atol=2e-6)
    primitive=b.antiderivative(plan,f)
    np.testing.assert_allclose(primitive[-1],b.integrate(plan,f),atol=1e-11)
...
def test_slice_dependent_boundary_jets(plan):
    ...
    result=jax.jit(b.decompose)(plan,f,boundary=bc)
...
def test_tensor_mixed_nonzero_derivative_and_batches():
    ...
    p=b.plan_3d(x,y,z,degree=3,n_basis=8,boundary_points=5)
    X,Y,Z=jnp.meshgrid(x,y,z,indexing='ij')
    f=X**2*Y*Z
    batch=jnp.stack([f,2j*f],axis=-1)
    out=jax.jit(partial(b.mixed_partial,orders=(1,1,1)))(p,batch)
...
def test_smooth_function_refinement():
    ...
    for n in (33,65,129):
        ...
        exact=jnp.exp(x)
        result=b.derivatives(p,exact,orders=(1,2))
