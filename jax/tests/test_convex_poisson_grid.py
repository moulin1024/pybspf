...
from bspf_jax.convex_poisson import ConvexPoissonPlan
from bspf_jax.convex_poisson_grid import _closed_domain_mask
...
def test_fixed_grid_matches_reference_and_reuses_factors(reference, monkeypatch):
    ...
    monkeypatch.setattr("bspf_jax.convex_poisson_grid.stream_evaluate_line", unexpected)
...
def test_grid_rejects_invalid_axes_before_solver_setup(monkeypatch):
    ...
    monkeypatch.setattr("bspf_jax.convex_poisson_grid.ConvexPoissonPlan", unexpected)
    domain = benchmark_domains()[0]
    for x, y in (
        ([], [0]),
        ([0, 0], [0]),
