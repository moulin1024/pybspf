...
def system(n=17, *, field=lambda z: 1+z*z/2, gradient=lambda z: z, mass=1.):
    ...
    vp = b.plan_1d(v, degree=5, n_basis=10, boundary_points=7)
    return b.plan_drift_kinetic(zp, vp, magnetic_field=field,
...
def test_harmonic_mirror_characteristics_and_all_face_balances_under_jit(mass):
    ...
    f, transfer = jax.jit(lambda p, f: b.integrate_drift_kinetic(
    ...
    moments = b.drift_kinetic_moments(p, f)
...
def test_energy_equilibrium_and_nonzero_mirror_force():
    ...
    df, rates = b.drift_kinetic_rhs(p, 0., f, inflow=equilibrium)
    ...
    df, _ = b.drift_kinetic_rhs(p, 0., tilted(0., z, v, mu), inflow=tilted)
...
def test_uniform_field_reduces_to_parallel_transport():
    ...
    times = jnp.array([0., .2])
    history, _ = b.integrate_drift_kinetic(p, f, times, inflow=lambda t,z,v,mu: 0., substeps=100)
    old = b.integrate_parallel_kinetic(p.transport, f[:, :, 0], times,
...
def test_validation():
    ...
    with pytest.raises(ValueError, match='initial'):
        b.integrate_drift_kinetic(p, jnp.ones((2, 3)), jnp.array([0.]), inflow=lambda t,z,v,mu: 0.)
...
def test_nonquadratic_field_energy_equilibrium():
    ...
    df, rates = b.drift_kinetic_rhs(p, 0., exact(0., z, v, mu), inflow=exact)
    np.testing.assert_allclose(df, 0., atol=2e-9)
    np.testing.assert_allclose(rates.sum(axis=-1), 0., atol=2e-9)
...
def test_log_transport_is_positive_between_nodes_and_has_physical_fluxes():
    ...
    g, transfers = jax.jit(lambda p, g: b.integrate_log_drift_kinetic(
    ...
    from bspf_jax.drift_kinetic import _boundary_fluxes, _boundary_rates
    gf = reference[-1]
...
def test_log_representation_does_not_interpolate_exponentiated_samples():
    ...
    with pytest.raises(ValueError, match='initial_log'):
        b.integrate_log_drift_kinetic(p, g[..., 0], jnp.array([0.]), log_inflow=lambda t,z,v,mu: 0.)
