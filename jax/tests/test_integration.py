...
def manufactured(dimension, sizes):
    ...
    plan = axes[0] if dimension == 1 else b.tensor_plan(*axes)
...
def test_nonperiodic_tensor_integration(dimension):
    ...
    integrate_box = jax.jit(b.integrate_box)
    reference = exact_box(full)
    np.testing.assert_allclose(
        b.integrate_box(plan, field), reference, atol=2e-10, rtol=2e-10
    )
    np.testing.assert_allclose(
    ...
    for axis, x in enumerate(coordinates):
        ...
        actual = jax.jit(partial(b.integrate, axis=axis))(plan, field, a=lo, b=hi)
        ...
        for order, expected in ((1, first), (2, second)):
            actual = jax.jit(partial(b.antiderivative, axis=axis, order=order))(
    ...
    def volume(upper):
        ...
        return b.integrate_box(plan, field, bounds)
