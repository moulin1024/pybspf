...
def test_chebyshev_map_matches_independent_least_squares(alpha):
    ...
    p = b.plan_1d(x, degree=5, n_basis=12, constraint_order=q,
...
def test_polynomial_jets_and_complex_jit_grad():
    ...
    p = b.plan_1d(x, degree=5, n_basis=12, endpoint_method="chebyshev",
    ...
    np.testing.assert_allclose(jax.jit(b.endpoint_jets)(p, f),
                               jnp.stack([exact, 2j*exact], axis=-1), atol=1e-9)
    df = jax.jit(b.differentiate)(p, f)
    np.testing.assert_allclose(df, jnp.stack([3*x*x, 6j*x*x], axis=-1), atol=1e-9)
    grad = jax.jit(jax.grad(lambda values: jnp.sum(b.differentiate(p, values))))(x**3)
    ...
    fd = b.plan_1d(x, degree=5, n_basis=12)
    np.testing.assert_allclose(b.differentiate(p, x**3, boundary=exact),
                               b.differentiate(fd, x**3, boundary=exact), atol=1e-12)


@pytest.mark.parametrize("dimension", [2, 3])
def test_tensor_factories_accept_estimator(dimension):
    x = jnp.linspace(-1, 1, 17)
    factory = b.plan_2d if dimension == 2 else b.plan_3d
    ...
    np.testing.assert_allclose(jax.jit(partial(b.differentiate, axis=dimension-1))(p, f),
                               2*mesh[-1], atol=1e-9)
...
def test_degree9_endpoint_option_improves_smooth_field():
    ...
    fd = b.plan_1d(x, boundary_points=9, **opts)
    local = b.plan_1d(x, endpoint_method="chebyshev", boundary_points=16,
                      chebyshev_modes=12, **opts)
    error_fd = jnp.linalg.norm(b.differentiate(fd, f)-exact)/jnp.linalg.norm(exact)
    error_local = jnp.linalg.norm(jax.jit(b.differentiate)(local, f)-exact)/jnp.linalg.norm(exact)
...
def test_compact_blocks_match_dense_reference(method, n):
    ...
    p = b.plan_1d(x, degree=5, n_basis=12, boundary_points=12,
    ...
    np.testing.assert_allclose(jax.jit(b.endpoint_jets)(p, jnp.asarray(f)),
    ...
    np.testing.assert_allclose(jax.jit(b.differentiate)(p, jnp.asarray(f)),
                               expected.reshape(f.shape), rtol=2e-11, atol=1e-8)
    # Boundary extraction also works on a non-leading physical axis.
    other = b.plan_1d(jnp.linspace(0, 1, 3), degree=1, n_basis=2)
    tensor = b.tensor_plan(other, p)
    np.testing.assert_allclose(b.endpoint_jets(tensor, jnp.asarray(f).swapaxes(0, 1), axis=1),
                               expected_jets, rtol=2e-12, atol=1e-8)
...
def test_empty_compact_jets(method):
    x = jnp.linspace(-1, 1, 17)
    p = b.plan_1d(x, degree=3, n_basis=8, constraint_order=0,
                  endpoint_method=method)
    assert p.boundary_blocks.shape[:2] == (2, 0)
    assert jax.jit(b.endpoint_jets)(p, x).shape == (2, 0)
    np.testing.assert_allclose(b.differentiate(p, jnp.ones_like(x)), 0, atol=1e-10)
...
def test_invalid_estimator_options(kwargs):
    with pytest.raises(ValueError):
        b.plan_1d(jnp.linspace(-1, 1, 33), **kwargs)
