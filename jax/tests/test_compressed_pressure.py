...
def test_pressure_rectangular_direct_and_projection():
    dense = b.plan_pressure_poisson2d(np.linspace(0, 1, 40), np.linspace(0, 1, 41))
    compressed = b.compress_pressure_plan(dense, tolerance=1e-12, leaf_size=8)
    ...
    rhs = b.pressure_schur(dense, p)
    gradient = b.pressure_gradient(dense, p)
    solve = jax.jit(partial(b.solve_pressure_poisson2d, refinement_steps=0))
    ...
    default = jax.jit(b.solve_pressure_poisson2d)(
    ...
    project = jax.jit(
        partial(b.project_pressure2d, completion=False, refinement_steps=0)
    )
    ...
    with pytest.raises(ValueError, match="already compressed"):
        b.compress_pressure_plan(compressed)
...
def test_factory_and_validation():
    ...
    assert bool(
        jax.jit(b.solve_pressure_poisson2d)(plan, jnp.zeros((12, 12))).converged
    )
    for options in [
        dict(tolerance=0),
    ...
    with pytest.raises(ValueError, match="transform_backend"):
        b.plan_pressure_poisson2d(grid, grid, transform_backend="bad")


def test_layered_zero_rank_and_single_leaf():
    from bspf_jax._compressed_transform import _layers
