...
def bulk_flux(config, ocean_velocity, air_velocity, sst, air_temperature, humidity):
...
def plan_air_sea(*, n=33, config=None, quadrature_order=None):
    ...
    zonal_wind = jnp.broadcast_to(
        -config.wind_speed * jnp.cos(2 * jnp.pi * y), weight.shape
    )
    ...
    target_air = target_sst - 2.0
    target_q = 0.75 * saturation_specific_humidity(target_air, config.pressure)
...
def exchange_loads(plan, ocean_velocity, sst, air_velocity, air_temperature, humidity):
    stress, sensible, water = bulk_flux(
        plan.config, ocean_velocity, air_velocity, sst, air_temperature, humidity
    )
    return Exchange(
