...
import argparse
ap=argparse.ArgumentParser();ap.add_argument('--nx',type=int,required=True);ap.add_argument('--ny',type=int,required=True);ap.add_argument('--degree',type=int,default=96);ap.add_argument('--laurent',type=int,default=64);ap.add_argument('--samples',type=int,default=800);args=ap.parse_args()
jax.config.update('jax_enable_x64',True)
device=jax.devices('gpu')[0]
suffix='' if args.degree==96 and args.laurent==64 and args.samples==800 else '_finer_extension'
root=Path(f'build/immersed_flow/re200_ripple_study/initial_{args.nx}x{args.ny}{suffix}');root.mkdir(parents=True,exist_ok=True)
p=ImmersedFlowPlan(assembly_device=device,basis_precision='float64',nx=args.nx,ny=args.ny,reynolds=200,wall_method='rational',quadrature_factor=2.5,rational_options=dict(degree=args.degree,laurent=args.laurent,samples=args.samples))
...
# Boundary leakage of the same initial acceleration, on independent points.
hole,_=p.arc.sample(256,offset=.371)
xwall=np.linspace(*p.bounds[:2],197);ywall=np.linspace(-1,1,157)
outer=np.vstack((np.column_stack((xwall,np.full_like(xwall,-1))),np.column_stack((xwall,np.full_like(xwall,1))),np.column_stack((np.full_like(ywall,p.bounds[0]),ywall))))
dc=p.transform@jax.device_get(da0)
b,r=evaluate_parts(dc,np.vstack((hole,outer)),affine=False);v=b+r
checks['initial_acceleration_hole_boundary_error']=float(np.max(np.hypot(v[1,:len(hole)],v[2,:len(hole)])))
checks['initial_acceleration_outer_boundary_error']=float(np.max(np.hypot(v[1,len(hole):],v[2,len(hole):])))
(root/'source_audit_checks.json').write_text(json.dumps(checks,indent=2)+'\n');print(json.dumps(checks),flush=True)
